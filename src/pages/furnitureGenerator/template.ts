...
export const tileTemplate: Record<string, TileTemplateFunction> = {
    Bed: (
        name: string,
        space: string,
        itemSpace: string,
        ...
        _light,
        dustNamespace: string
    ) => /* cs */ `using Terraria.DataStructures;
...
public class ${name} : ModTile
{
	...
	public override void SetStaticDefaults() {
		...
		DustType = ModContent.DustType<${dust}>(); // You should set a kind of dust manually.
		AdjTiles = new int[] { TileID.Beds };

		// Placement
		TileObjectData.newTile.CopyFrom(TileObjectData.Style4x2); // this style already takes care of direction for us
		TileObjectData.newTile.CoordinateHeights = new[] { ${coord.join(', ')} };
...
public class ${name}Closed : ModTile
{
	public override void SetStaticDefaults() {
		...
		TileID.Sets.HasOutlines[Type] = true;
		TileID.Sets.DisableSmartCursor[Type] = true;
		TileID.Sets.OpenDoorID[Type] = ModContent.TileType<${name}>();
...
public class ${name} : ModTile, ITileFluentlyDrawn${
        flame.length > 0 ? ', ITileFlameData' : ''
    }
{
	${flame.length > 0 ? `private Asset<Texture2D> flameTexture;` : ''}
	...
	public override void ModifyLight(int i, int j, ref float r, ref float g, ref float b)
	{
		...
		{
			r = ${lightColor[0].toFixed(3)}f;
			g = ${lightColor[1].toFixed(3)}f;
			b = ${lightColor[2].toFixed(3)}f;
		}
		else
		{
			r = 0f;
	...
	public override void NearbyEffects(int i, int j, bool closer)
	{
		...
		if (tile.TileFrameX < 54)
		{
			...
			if (!Main.gamePaused && Main.instance.IsActive && (!Lighting.UpdateEveryFrame || Main.rand.NextBool(4)) && Main.rand.NextBool(frequency))
			{
				...
				int numForDust = Dust.NewDust(dustBox.TopLeft(), dustBox.Width, dustBox.Height, ModContent.DustType<${dust}>(), 0f, 0f, 254, default, Main.rand.NextFloat(0.95f, 1.75f));
...
public class ${name} : ModTile
{
	...
	public override void MouseOver(int i, int j)
	{
		FurnitureUtils.SofaMouseOver<${itemSpace}.${name}>(i, j);
	}
}`
};
